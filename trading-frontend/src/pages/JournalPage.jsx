...
function getEntryDate(entry) {
  const value = entry.created_at || entry.created || entry.timestamp || entry.date;
...
export default function JournalPage() {
  ...
  async function fetchEntries() {
    ...
    try {
      ...
      setEntries(normalizeList(res.data));
    } catch (err) {
...
`.trim();
    ...
    if (!saved) {
      setError(getApiErrorMessage(lastError));
      setLoading(false);
      return;
    }
  ...
  const stats = useMemo(() => {
    ...
    const followed = entries.filter((entry) =>
      getEntryText(entry).toLowerCase().includes("rule followed | yes")
    ).length;

    const wins = entries.filter((entry) =>
      getEntryText(entry).toLowerCase().includes("outcome | win")
    ).length;

    const losses = entries.filter((entry) =>
      getEntryText(entry).toLowerCase().includes("outcome | loss")
    ).length;
        ...
        <div className="relative z-10 grid gap-8 xl:grid-cols-[1fr,0.8fr]">
          <div>
            ...
            <p className="mt-5 max-w-3xl text-lg leading-8 text-slate-300">
              ...
              invalidation, execution mistakes, and outcome. The journal is your
              trading system’s feedback loop.
            </p>
          </div>
            ...
            <div className="space-y-4">
              {entries.map((entry) => (
                ...
                >
                  ...
                  <MarkdownRenderer content={getEntryText(entry)} />
                </div>
              ))}
            </div>
          )}
          ...
          <Card title="Review Prompts" subtitle="Questions serious traders ask">
            <div className="space-y-3">
              {[
                ...
                "Was this a planned trade or impulse?",
                "Did session context support the setup?",
                "What mistake should I eliminate next?",
              ].map((prompt) => (
                <div
                  key={prompt}
